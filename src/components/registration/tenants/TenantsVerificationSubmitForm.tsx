...
interface TenantsVerificationSubmitFormProps {
  formData: {
    email: string;
    address: {
      flatOrHouseNo: string;
...
export function TenantsVerificationSubmitForm({
  formData,
  selectedFiles,
  setSelectedFiles,
  onBack,
}: TenantsVerificationSubmitFormProps) {
  ...
  // Update form fields when profileData changes
  useEffect(() => {
    if (profileData?.data) {
      ...
      setValue("guarantor", profileData.data.guarantor || formData.guarantor, { shouldValidate: true });
      setValue("references", profileData.data.references || formData.references, { shouldValidate: true });
    }
  }, [profileData, setValue, formData]);
  ...
  const handleUpload = async (files: File[]): Promise<string[]> => {
    ...
    // Return an empty array or implement logic to return uploaded file URLs/IDs as needed
    return [];
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const payload = {
        ...
        lookingPropertyForTenant: data.lookingPropertyForTenant || [],
      ...
      <div className="container mx-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <Label className="text-sm font-medium text-gray-700">Guarantor Details</Label>
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    {errors.guarantor?.name && (
                      <p className="text-xs text-red-500">{errors.guarantor.name.message}</p>
                    )}
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    {errors.guarantor?.email && (
                      <p className="text-xs text-red-500">{errors.guarantor.email.message}</p>
                    )}
                  </div>
                  <div className="space-y-1">
            ...
            <div className="space-y-4">
              {formData.references?.map((_, index) => (
                <div key={index} className="space-y-1">
                  <Label className="text-sm font-medium text-gray-700">Reference {index + 1} Details</Label>
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <Label className="text-sm font-medium text-gray-500 mb-1">Name</Label>
                      <Input
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                      />
                      {errors.references?.[index]?.telephone && (
                        <p className="text-xs text-red-500">{errors.references[index].telephone?.message}</p>
                      )}
                    </div>
                    <div className="space-y-1">
                      <Label className="text-sm font-medium text-gray-500 mb-1">Email</Label>
                      ...
                      {errors.references?.[index]?.email && (
                        <p className="text-xs text-red-500">{errors.references[index].email?.message}</p>
                      )}
                    </div>
                    <div className="space-y-1">
                      <Label className="text-sm font-medium text-gray-500 mb-1">Profession</Label>
                      <Input
                        {...register(`references.${index}.profession`)}
                        placeholder="Profession"
                        className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                      />
                      {errors.references?.[index]?.profession && (
                        <p className="text-xs text-red-500">{errors.references[index].profession?.message}</p>
                      )}
                    </div>
                    <div className="space-y-1">
                      <Label className="text-sm font-medium text-gray-500 mb-1">Address</Label>
                      <Input
                        {...register(`references.${index}.address.address`)}
                        placeholder="Address"
                        className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                      />
                      {errors.references?.[index]?.address?.address && (
                        <p className="text-xs text-red-500">{errors.references[index].address?.address?.message}</p>
                      )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        ...
                        <StateInput
                          form={form}
                          name={`references.${index}.address.state`}
                          placeholder="State"
                          className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                        />
                        {errors.references?.[index]?.address?.state && (
                          <p className="text-xs text-red-500">{errors.references[index].address?.state?.message}</p>
                        )}
                      </div>
                      <div className="space-y-1">
                        <Label className="text-sm font-medium text-gray-500 mb-1">City</Label>
                        <CityInput
                          form={form}
                          name={`references.${index}.address.city`}
                          placeholder="City"
                          className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                        />
                        {errors.references?.[index]?.address?.city && (
                          <p className="text-xs text-red-500">{errors.references[index].address?.city?.message}</p>
                        )}
                      </div>
                      <div className="space-y-1">
                        <Label className="text-sm font-medium text-gray-500 mb-1">Town</Label>
                        <Input
                          {...register(`references.${index}.address.town`)}
                          placeholder="Town"
                          className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                        />
                        {errors.references?.[index]?.address?.town && (
                          <p className="text-xs text-red-500">{errors.references[index].address?.town?.message}</p>
                        )}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <Label className="text-sm font-medium text-gray-500 mb-1">Area</Label>
                      <Input
                        {...register(`references.${index}.address.area`)}
                        placeholder="Area code"
                        className="h-12 border-gray-300 rounded-lg focus:border-primary focus:ring-primary"
                      />
                      {errors.references?.[index]?.address?.area && (
                        <p className="text-xs text-red-500">{errors.references[index].address?.area?.message}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="rounded-lg py-8 mb-8">
              ...
              <div className="space-y-4 text-gray-600">
                ...
                <p>Unverified profiles will remain active but may receive fewer responses.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button
                ...
                onClick={onBack}
                disabled={isUploading || isUpdating || isSubmitLoading}
              >
                Back
              </Button>
              <Button
