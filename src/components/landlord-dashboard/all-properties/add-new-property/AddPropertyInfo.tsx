...
const baseFormSchema = z.object({
  headlineYourProperty: z.string().trim(),
...
export default function AddPropertyInfo() {
  ...
  const [imagesRawFile, setImagesRawFile] = useState<any>([])
  ...
  useEffect(() => {
    ...
    if (storedData) {
      setValue("headlineYourProperty", storedData.headlineYourProperty || "")
      setValue("propertyType", storedData.propertyType || "")
      setValue("bedrooms", storedData.bedrooms || 0)
      setValue("bathrooms", storedData.bathrooms || 0)
      setValue("livingRooms", storedData.livingRooms || 0)
      setValue("kitchen", storedData.kitchen || 0)
      setValue("description", storedData.description || "")
      setValue("images", storedData.images || [])
      setValue("status", storedData.status || "available")
      setValue("gender", storedData.gender || "")
      setValue("features", storedData.features || [])
      setValue("formAvailable", storedData.formAvailable || "")
      setValue("furnished", storedData.furnished || undefined)
      setValue("ages", storedData.ages || "")
      setValue("rentPerYear", storedData.rentPerYear || 0)
      setValue("rentPerMonth", storedData.rentPerMonth || 0)
      setValue("serviceCharge", storedData.serviceCharge || 0)
      setValue("depositAmount", storedData.depositAmount || undefined)
      setValue("isIncludeAllUtilityWithService", storedData.isIncludeAllUtilityWithService || false)
      setValue("minimumLengthOfContract", storedData.minimumLengthOfContract || undefined)
      setValue("isReferenceRequired", storedData.isReferenceRequired || false)
      setValue("accessYourProperty", storedData.accessYourProperty || [])
      setValue("mediaLink", storedData.mediaLink || "")
      setValue("isAcceptTermsAndCondition", storedData.isAcceptTermsAndCondition || false)
      setValue("isRemoteVideoView", storedData.isRemoteVideoView || false)

      // Update state for selected features, access types, and images
      setSelectedFeatures(storedData.features || [])
      setSelectedAccessTypes(storedData.accessYourProperty || [])
      setImageFiles(storedData.images || [])
    }
  }, [setValue])
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      setImagesRawFile([...imagesRawFile, ...Array.from(files)])
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      ...
      if (imagesRawFile.length > 0) {
        ...
        imagesRawFile.forEach((file: any) => formData.append("images", file))

        const uploadResponse = await uploadFile(formData).unwrap().then((res) => res).catch((error) => {
      ...
      if (propertyFormData?.location) {
        newData.location = propertyFormData.location
      }
  ...
  return (
    <div className="w-full max-w-4xl lg:p-8 rounded-lg mb-12 bg-white">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Furnished */}
        <div>
          <Select onValueChange={(value) => setValue("furnished", value as "fully_furnished" | "unfurnished" | "semi_furnished")}>
            <SelectTrigger className="h-12 border-gray-300 focus:border-gray-400 focus:ring-0">
        ...
        <div className="flex justify-end gap-4 pt-6">
          ...
          <Button type="submit" className="cursor-pointer px-6 py-2 bg-primary hover:bg-primary text-white">
            {isUploading ? <LoaderIcon className="animate-spin" /> : "Next"}
          </Button>
        </div>
      </form>
    </div>
