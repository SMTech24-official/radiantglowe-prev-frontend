...
import { useAddMessageInTicketMutation, useGetAllTicketQuery, useUpdateTicketStatusMutation } from "@/redux/api/supportApi";
...
export default function AdminTicketPage() {
  ...
  const filteredData = tickets?.filter((item: any) => {
    const matchesSearch =
  ...
  useEffect(() => {
    if (isMessagesModalOpen && messagesEndRef.current) {
      messagesEndRef.current.scrollIntoView({ behavior: "smooth" });
    }
  }, [isMessagesModalOpen, selectedTicket?.messages]);
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            ...
            <Select onValueChange={setStatusFilter} defaultValue="all">
              <SelectTrigger className="w-full sm:w-40 border-gray-300 focus:border-primary focus:ring-primary">
              ...
              <div className="space-y-4 max-h-[50vh] overflow-y-auto">
                {selectedTicket?.messages?.map((msg: any, index: number) => (
                  <div
                    ...
                    ref={index === selectedTicket.messages.length - 1 ? messagesEndRef : null}
                  >
